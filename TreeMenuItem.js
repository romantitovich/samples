...
import { FS_URL } from "../../../app/constants/backend";
/**
...
class TreeMenuItem extends Component {
		...
		if (itemShowIcon)
			return (
				<View style={{ width: iconSize + iconSize * 0.3, marginLeft: 0, marginRight: 0 }}>
					<Text style={{ color: color, fontSize: 20, alignSelf: 'center' }}>{unicodeChar}</Text>
				</View>
			);
		else
			return (<View />);
	...
	componentDidMount() {
		...
		// console.log("LLLL",menuItemObject.EventId)
		//let menuItemObject = this.props.menuItemObject;
		//if (this.props.keyTree == 1 && menuItemObject.EventId == 1) {
		//	menuItemObject.openSubMenu == true;
		//let menuItemObjectParent = menuItemObject.parent;
			//this.props.onOpenSubMenu(menuItemObjectParent)
			//this.props.onOpenEventMenu(menuItemObject)
		//}
	}
		...
		return (
			<View style={{ flex: 1 }}>
				<TouchableHighlight
					...
					value={menuItemObject.block_id}
					underlayColor="#00000000"
					onPress={() => {
						if (menuItemObject.onClick !== undefined && menuItemObject.custom_menu_block != null && menuItemObject.custom_menu_block.length > 0) {
					...
					<View
						...
						<Image style={{ width: 15, height: 15, paddingRight: 15, resizeMode: "cover" }} source={{
							uri: FS_URL + "/files/images/" + menuItemObject.block_icon_name,
							method: "GET",
							headers: {
								"Authorization": this.props.bearerToken.toString()
							}
						}} />

...
class TreeMenu extends Component {
	...
	setSubItemParent(menuObject, parent) {
		...
		if (menuObject.custom_menu_block) {
			for (let i = 0; i < menuObject.custom_menu_block.length; i++) {
				this.setSubItemParent(menuObject.custom_menu_block[i], menuObject);
			}
		}
	}
	...
	renderMenu(level, key, menuObjects) {
		...
		if (menuObjects !== undefined && menuObjects.length > 0) {
			for (let i = 0; i < menuObjects.length; i++) {
				...
				//if (menuItemObject.block_id == 10001){
				//	menuItemObject.openSubMenu = true;
				//}
				let custom_menu_block = menuItemObject.custom_menu_block;
				let showDropDownButton = false;
				if (custom_menu_block != null && custom_menu_block.length > 0) {
				...
				returnValue.push(
					<View
						...
						<TreeMenuItem
							...
							onOpenEventMenu={(menuItemObject)=>{
								...
								if (this.props.itemOpenCloseHandler)
									...
									// Close others:
										...
										menuItemObjectParent.openSubMenu = true;
							...
							onOpenSubMenu={(menuItemObject) => {
								...
								if (menuItemObject.openSubMenu) {
									...
									if (this.props.menuItemSettings.closeOthersOnOpen) {
										...
										let menuItemObjectParent = menuItemObject.parent;
										if (menuItemObjectParent)
											itemsToCompress = menuItemObjectParent.custom_menu_block?menuItemObjectParent.custom_menu_block:[];
				...
				if (menuItemObject.openSubMenu === true) {
					// console.log("GGG",custom_menu_block)
					if (custom_menu_block !=null && custom_menu_block.length > 0) {
						returnValue.push(this.renderMenu(level + 1, key, custom_menu_block));
					}
				}
			}
		}
